#!/usr/bin/env bash
# Post-gatefix actor FT on 125: gate-passed WM + actor collect (NO closed_loop).
# CL planner is deferred until actor-only SR shows the head is usable online.
#
#   bash experiments/aerial/scripts/run_directional_oa_actor_ft_125.sh
set -euo pipefail
...
echo "=== actor-ft-125 stamp=$STAMP (no closed_loop; gatefix WM) ==="
hostname
...
if [[ -n "${WM:-}" && -f "$WM" ]]; then
  ...
  echo "FATAL: need gate-passed WM in LATEST_REFLOW_CKPT.txt"; exit 1
fi
test -f "$WM"

GATE_JSON="$ART/obstacle_cost_gt_depth/gate_gatefix_20260922_161641.json"
if [[ -f "$GATE_JSON" ]]; then
  "$PY" - <<PY
import json
from pathlib import Path
r=json.loads(Path("$GATE_JSON").read_text())
assert r.get("passed") is True, r
print("gate_ok", {k:v for k,v in r.get("checks",{}).items()})
PY
else
  echo "WARN: missing $GATE_JSON — proceeding with LATEST WM only"
fi
...
"$PY" -m experiments.aerial.rl.train_v4_ac \
  ...
  2>&1 | tee -a "$ART/logs/directional_oa_actor_ft_train_${STAMP}.log"
...
run_eval() {
  ...
  "$PY" -m experiments.aerial.scripts.wam_phase2_long_eval \
    ...
    --min-spawn-clear-m 5.0 \
    "$@" \
    > "$ART/logs/directional_oa_actor_ft_eval_${TAG}_${STAMP}.log" 2>&1 || true
  "$PY" - <<PY
import json
from pathlib import Path
...
echo "=== ACTOR-FT-125 DONE stamp=$STAMP actor=$ACTOR wm=$WM ==="
