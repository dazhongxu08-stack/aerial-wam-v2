#!/usr/bin/env bash
# Smoke-record dual-view for 20 R09-template interior routes (V11 stack, not V12).
set -euo pipefail
...
OUT_ROOT="${OUT_ROOT:-artifacts/wam_phase2_v11_interior20_smoke_20260916}"
...
echo "=== interior20 smoke (V11) routes=[$ROUTES] -> $OUT_ROOT ===" | tee "$LOG"
...
for idx in "${ROUTE_ARR[@]}"; do
  ...
  "$PY" -m experiments.aerial.scripts.wam_phase2_dual_view_from_traj \
    ...
    --route-idx "$idx" \
    --plan-mode polyline \
    --route-label "V11 interior ${label}" \
    --out-dir "$OUT_ROOT/videos" \
    --out-prefix "V11_interior_${label}" \
    --fps 5 --source-hz 5 2>&1 | tee -a "$LOG"
done

echo "[$(date '+%H:%M:%S')] interior20 smoke batch done (V11)" | tee -a "$LOG"
