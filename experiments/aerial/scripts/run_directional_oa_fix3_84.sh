#!/usr/bin/env bash
# Directional-OA fix3 on **84 only** (reverse-behind tax + motion-aligned imag blend).
# Starts from hbclear (NOT collapsed hard014). Reuses prior reflow WM when present.
#
#   WAIT_FIX2=1 bash experiments/aerial/scripts/run_directional_oa_fix3_84.sh
set -euo pipefail
...
echo "=== fix3-84 stamp=$STAMP root=$ROOT (always tax dx<0; imag blend action-aligned) ==="
hostname
nvidia-smi --query-gpu=name,memory.used,memory.total --format=csv,noheader
ss -ltnp | grep 41451 || { echo "FATAL AirSim :41451 down"; exit 1; }

if [[ "${WAIT_FIX2:-0}" == "1" ]]; then
  echo "waiting for fix2_84 / train_v4_ac / long_eval to free GPU..."
  while pgrep -af 'fix2_84|train_v4_ac|wam_phase2_long_eval' | grep -v pgrep | grep -v fix3 >/dev/null; do
    sleep 30
  done
  echo "GPU idle for fix3"
fi
...
pick_actor() {
  ...
  elif [[ -f "$d/v4_ac_best_sr.pt" ]]; then echo "$d/v4_ac_best_sr.pt"
  else return 1
  fi
}

# Prefer non-collapsed OA warm start: best_sr2 > hbclear > easy > prior fix2
BEST_SR2="$ART/v4_ac_ckpt_urban_directional_oa_20260922_022358_best_sr2"
INIT="$(pick_actor "$BEST_SR2" || pick_actor "$HBCLEAR" || true)"
if [[ -z "${INIT:-}" ]]; then
  INIT="$(pick_actor "$ART/v4_ac_ckpt_urban_directional_oa_20260922_141414_easy" || pick_actor "$ART/v4_ac_ckpt_urban_directional_oa_20260922_151034_fix2_84" || true)"
fi
[[ -n "${INIT:-}" ]] || { echo "FATAL no INIT actor"; exit 1; }
echo "INIT=$INIT"

# Reuse reflow WM if present; else reflow
if [[ -n "${WM:-}" && -f "$WM" ]]; then
  echo "WM (env)=$WM"
  ...
  echo "FATAL no WM and SKIP_REFLOW=1"; exit 1
fi
test -f "$WM"
...
"$PY" -m experiments.aerial.rl.train_v4_ac \
  ...
  2>&1 | tee -a "$ART/logs/directional_oa_fix3_84_train_${STAMP}.log"
...
run_eval() {
  ...
  "$PY" -m experiments.aerial.scripts.wam_phase2_long_eval \
    ...
    "$@" \
    > "$ART/logs/directional_oa_fix3_84_eval_${TAG}_${STAMP}.log" 2>&1 || true
  "$PY" - <<PY
import json
from pathlib import Path
...
print("actor=$ACTOR wm=$WM init=$INIT")
print("=== FIX3-84 DONE ===")
PY
